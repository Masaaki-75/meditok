...
def main(args):
    ...
    latent_size = 16 #args.image_size // args.downsample_size
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            x = x.to(device, non_blocking=True)  # [batch_size, 8, 16, 16]
            y = y.to(device, non_blocking=True)
            c_indices = y.reshape(-1)  # shape: [B,]
            z_indices = x.reshape(x.shape[0], x.shape[1], -1)  # [B, K=8, 256]
            #print(f"----> z_indices={z_indices.shape}, c_indices={c_indices.shape}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocab-size", type=int, default=32768, help="vocabulary size of visual tokenizer")  # 32768是8个码本的总和
    ...
    parser.add_argument("--dataset", type=str, default='c2i_code')
